...
export default function CartPage() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const orderNumber = result.placeOrderTaskId || result.orderId;
      // Gọi API lấy link thanh toán VNPay
      const vnpayUrl = await paymentService.createPaymentUrl(orderNumber);
      
      if (vnpayUrl) {
        window.location.href = vnpayUrl;
      } else {
        setError('Lỗi tạo link thanh toán VNPay');
        setLoading(false)
      }
    } catch {

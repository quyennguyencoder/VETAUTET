...
INSERT INTO `vetautet`.`ticket` (`name`, `description`, `start_time`, `end_time`, `status`, `updated_at`, `created_at`)
VALUES
...
-- 5. payment table
CREATE TABLE IF NOT EXISTS `vetautet`.`payment_transaction` (
...
CREATE TABLE IF NOT EXISTS `vetautet`.`outbox_event` (
    `id`           BIGINT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS `vetautet`.`idempotency_key` (
    `token`      VARCHAR(64) NOT NULL COMMENT 'Unique token từ PlaceOrderMQMessage',
    `created_at` DATETIME    NOT NULL COMMENT 'Thời điểm consumer nhận lần đầu',
    `expires_at` DATETIME    NOT NULL COMMENT 'TTL — dùng cho cleanup job',
    PRIMARY KEY (`token`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = 'Idempotency gate — INSERT IGNORE prevents duplicate processing';

CREATE INDEX idx_idempotency_expires_at ON `vetautet`.`idempotency_key` (`expires_at`);
